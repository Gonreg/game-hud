...
## wallet.bonus_wager_* и wallet.bonus_expires (отыгрыш и срок бонуса, 1.2.0–1.3.0)

Ключи `wallet.bonus_wager_info`, `wallet.bonus_wager_left` (1.2.0) и
`wallet.bonus_expires` (1.3.0, «Бонус сгорит {{date}}») переведены только на `en` и
`ru`: механика отыгрыша и срока сгорания появилась в молоте, и переводить её
условия было не с чего — ни в одной из игр таких строк раньше не было.

Для остальных восьми языков (`es`, `de`, `fr`, `hi`, `ur`, `bn`, `si`, `ne`) подставлен
английский текст — по той же причине, что и везде в этом файле: полагаться на
`fallbackLng` нельзя, он разный по играм. Дата в `wallet.bonus_expires` при этом
форматируется на языке интерфейса (`formatDeadline`), так что непереведённой
остаётся только подпись вокруг неё.

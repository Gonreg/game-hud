...
Задачи идут по номерам **с одним исключением: Task 9 (словари i18n) выполняется сразу
после Task 3**, до Task 4. Причина: тестовый хелпер `renderWithHud` из Task 4 импортирует
`src/i18n/locales/en.json`, а тесты экранов ждут настоящих английских строк. Пока словарей
нет, `npm run typecheck` спотыкается на несуществующем модуле, хотя `vitest` этого и не
замечает — компилирует только то, что реально импортировано. Зависимостей у Task 9 нет ни
от одной другой задачи, поэтому дешевле переставить её, чем оставлять грабли.

Фактический порядок: **1 → 2 → 3 → 9 → 4 → 5 → 6 → 7 → 8 → 10 → 11 → … → 26.**
...
- **T7 — точка с запятой и кавычки.** Исходники fatman в стиле prettier с точками
  с запятой и одинарными кавычками — сохраняем его во всём пакете. Конкретно:
  `semi: true`, `singleQuote: true`, `trailingComma: 'all'`, `printWidth: 100`,
  `tabWidth: 2`, `arrowParens: 'always'` — это дословно `fatman/frontend/.prettierrc`.

  **Не запускай `npx prettier --check`**: файла конфига в репозитории нет, и голый
  prettier применит свои дефолты (двойные кавычки), поэтому наругается на весь
  проект — включая уже отревьюированные файлы. Это ложное срабатывание, а не
  находка. Сверяйся с соседними файлами в `src/`.
...
Хелпер импортирует `../i18n/locales/en.json` — он существует, потому что Task 9
выполняется раньше этой задачи (см. «Порядок исполнения» в начале плана). Если ты
почему-то делаешь Task 4 до Task 9, останови и сделай сначала Task 9, иначе
`npm run typecheck` упадёт на несуществующем модуле.
...
**Переменную масштаба надо выставлять на `document.documentElement`.** Хук читает
её именно оттуда, а CSS-переменные не всплывают вверх по дереву: если игра ставит
её на `#stage`, хук получит пустую строку и молча примет масштаб за единицу.
Так и случилось при интеграции matreshka. Теперь в dev-сборке об этом печатается
ошибка с объяснением, но лучше сразу дублировать переменную на корень.
...
> Исполняется сразу после Task 3, до Task 4 — см. «Порядок исполнения» в начале плана.
...
**Осознанное отклонение от fatman.** Там переключатель — два `<div>` с `onClick`,
то есть недоступен с клавиатуры и не объявляется скринридером. В библиотеке это
скрытый нативный `<input type="checkbox">` внутри `<label>` рядом с прежним
визуальным `<span>`. Внешний вид не меняется: галочка рисуется через `::after` на
спане, а на `<input>` псевдоэлементы не работают — потому в fatman и стоял `div`.
Библиотека едет в шесть игр, поэтому доступность окупается шестикратно.

**Второе отклонение — `LanguageScreen` сам выставляет `document.documentElement.lang`
и `dir`.** В fatman это делает слушатель `languageChanged` в его собственном
`i18n/index.ts`, но инстанс i18next принадлежит игре, и библиотека не может
рассчитывать, что такой слушатель есть у всех шести. Логика перенесена в экран.
Игре свой слушатель оставить: он нужен для начального языка, который выставляется
до открытия кабинета.

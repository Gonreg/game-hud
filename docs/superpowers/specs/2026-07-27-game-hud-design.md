...
**fatman — самая полная реализация, и код берём из него.** Даты коммитов вводят в
заблуждение: фронт fatman заморожен с 3 июня 2026, а matreshka и crash-race правились по
26 июля — но июльские коммиты crash-race («1:1 with jubb», «like jubb») означают, что
crash-race *догонял* fatman. Проверка по коду, а не по сообщениям коммитов:

| фича | fatman | matreshka |
|---|---|---|
| 11 типов уведомлений | есть | есть |
| селекторы метрики и окна в лидерборде | есть | есть |
| markdown-рендер Terms/Privacy/Offer | есть | есть |
| промо-карусель в кошельке | нет | **есть** |
| живой адрес TonConnect в строке кошелька Hub | нет | **есть** |

По объёму fatman заметно богаче: WalletScreen 211 строк против 144, HelpScreen 266 против
147, StatsScreen 169 против 91. Остальные расхождения — это переписывание matreshka под
свой api-клиент и свои типы, а не новая функциональность.

**Правило:** база — fatman, поверх неё четыре точечных улучшения из matreshka:

1. `ProfileHub` — живой `useTonAddress()` вместо `me.walletAddress` (на сервере это поле
   всегда пустое, потому что кошелёк подключается на клиенте)
2. `WalletScreen` — промо-карусель
3. `LegalScreen` — markdown собирается в React-узлы вместо `dangerouslySetInnerHTML`
4. store — `openWalletWithFocus` заодно выставляет `open: true` (в fatman оверлей остаётся
   закрытым — баг), плюс метод `openHistory()`

Набор иконок — объединение: у matreshka и basketball их 140 строк против 127 у fatman.
...
5. В браузере кабинет fatman не отличается от текущего прода — **кроме трёх ожидаемых
   отличий**, которые приходят вместе с улучшениями из matreshka: живой адрес TonConnect
   в строке кошелька на Hub, промо-карусель в кошельке, и открытие кошелька из
   баланс-пилюли теперь действительно раскрывает оверлей. Любое другое отличие — баг
   библиотеки.

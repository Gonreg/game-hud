...
export default defineConfig({
  ...
  test: {
    environment: 'jsdom',
    globals: true,
    // renderWithHud поднимает отдельный инстанс i18next на каждый рендер, чтобы
    // выбранный язык не протекал между тестами. Это правильно, но небесплатно:
    // при дефолтных пяти секундах и параллельных воркерах экранные тесты начинают
    // падать по таймауту на загруженной машине. Тесты не висят — они медленные,
    // поэтому поднимаем порог, а не прячем проблему отключением параллелизма.
    testTimeout: 20000,
